...
function AuthorCard({ authors, user, setAuthors }) {
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await axios.put(`https://87.228.10.180/api/authors/change_author/${authors.id}`, requestData, {
                headers: {
    ...
    const handleDelete = async () => {
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const response = await axios.delete(`https://87.228.10.180/api/admin/authors/delete_author/${authors.id}`, {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem('user_access_token')}`,
                        },
                    });
